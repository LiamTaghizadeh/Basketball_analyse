...
این پروژه با استفاده از پشته‌ی کامل **Python Data Ecosystem** توسعه یافته است:

### 🌐 فاز ۱: Web Scraping
- **`Requests`** : ارسال درخواست‌های HTTP به سرور
- **`BeautifulSoup4`** : تجزیه و استخراج داده از ساختار HTML
- **`Selenium`** : شبیه‌سازی مرورگر برای محتوای داینامیک (جاوااسکریپتی)
- **`Time` / `Random`** : مدیریت تأخیر و جلوگیری از مسدودسازی IP

### 🗄 فاز ۲: پایگاه داده و ETL
- **`Python 3.9+`** : زبان اصلی برنامه‌نویسی
- **`SQLAlchemy (ORM)`** : لایهٔ انتزاع پایگاه داده و مدیریت مدل‌ها
- **`Pandas`** : پردازش، پاکسازی و تبدیل دیتافریم‌ها
- **`PyMySQL`** : درایور اتصال به MySQL
- **`python-dotenv`** : مدیریت متغیرهای محیطی (اطلاعات ورود به دیتابیس)

### 📊 فاز ۳: تحلیل داده
- **`NumPy`** : محاسبات عددی برداری
- **`Scikit-learn`** : پیش‌پردازش، خوشه‌بندی و مدل‌سازی (در صورت نیاز)
- **`SciPy`** : انجام آزمون‌های آماری پیشرفته

### 📈 فاز ۴: بصری‌سازی و دشبورد
- **`Matplotlib` / `Seaborn`** : رسم نمودارهای ایستا و حرفه‌ای
- **`Plotly`** : نمودارهای تعاملی و پویا
- **`Streamlit`** : ساخت سریع اپلیکیشن‌های وب داده‌محور

### 🛡 مدیریت و همکاری
- **`Git`** : کنترل نسخه
- **`GitHub`** : میزبانی کد و همکاری تیمی

---

## 📊 داشبورد تحلیلی (Dashboard)

در این پروژه، یک داشبورد تحلیلی کامل با استفاده از **HTML، CSS و JavaScript** طراحی شده است که تمام آزمون‌های آماری، نمودارهای توزیع، ماتریس همبستگی و اندازه اثر را به صورت تعاملی نمایش می‌دهد.

### تصویر : نمای کلی داشبورد
![نمای کلی داشبورد](img/Screenshot From 2026-08-10 11-38-24.png)

![آزمون فرضیه و نمودارها](img/Screenshot From 2026-08-10 11-38-40.png)

> 💡 برای مشاهده و اجرای داشبورد، فایل `index.html` را در مرورگر باز کنید.
