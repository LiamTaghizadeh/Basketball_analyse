...
class Season_Player_CLub(Data_Cleaner):
        def __init__(self, dataframe):
            super().__init__(dataframe)
        ...
        def clean_season_column(self, column_name='season_id'):
            df_cleaned = self._dataframe.copy()
            ...
            valid_rows = df_cleaned[column_name].str.match(season_pattern, na=False)
            df_cleaned = df_cleaned[valid_rows]
            self._dataframe = df_cleaned 
            return self._dataframe